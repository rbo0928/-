...
BEST_MODEL_SAVE_PATH = 'beta9_at_1015.pth'
...
# 優化後的模型與訓練參數
SEQUENCE_LENGTH = 10  # 減少序列長度以節省記憶體
BATCH_SIZE = 20        # 針對8G顯卡優化
EPOCHS = 800
...
class DrivingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        last_valid_wheel = None

        for i in range(start_index, end_index):
            # 邊界檢查
            if i >= len(self.annotations):
                if last_valid_wheel is not None:
                    wheel_sequence.append(last_valid_wheel)
                else:
                    wheel_sequence.append([float(l_speed), float(r_speed)])
            ...
            wheel_pair = [float(row['lwheel']), float(row['rwheel'])]
            wheel_sequence.append(wheel_pair)
            last_valid_wheel = wheel_pair
        ...
        wheel_tensor = torch.tensor(wheel_sequence, dtype=torch.float32)
        return images_tensor, wheel_tensor, targets
...
class LaneCenteringController(nn.Module):
    def __init__(self, d_model, nhead, num_encoder_layers, dropout, seq_len, num_classes=2, hidden_factor=2):
        super().__init__()
        ...
        self.speed_proj = nn.Linear(2, d_model)
        ...
        self.pos_encoder = PositionalEncoding(d_model, dropout=dropout, max_len=seq_len + 5)

        self.cls_token = nn.Parameter(torch.zeros(1, 1, d_model))

        self.head = nn.Sequential(
            nn.LayerNorm(d_model),
            nn.Linear(d_model, d_model * hidden_factor),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(d_model * hidden_factor, num_classes)
        )

    def forward(self, frames: torch.Tensor, wheel_speeds: torch.Tensor) -> torch.Tensor:
        # frames: (batch, seq_len, C, H, W)
        # wheel_speeds: (batch, seq_len, 2)
        batch_size, seq_len, c, h, w = frames.shape

        if seq_len != self.seq_len:
            raise ValueError(f"序列長度 {seq_len} 與模型設定的 {self.seq_len} 不符。請調整 SEQUENCE_LENGTH 或資料集。")

        cnn_input = frames.view(batch_size * seq_len, c, h, w)
        image_features = self.cnn(cnn_input).view(batch_size, seq_len, self.d_model)

        speed_features = self.speed_proj(wheel_speeds)
        combined = image_features + speed_features

        cls_tokens = self.cls_token.expand(batch_size, -1, -1)
        token_sequence = torch.cat([cls_tokens, combined], dim=1)
        token_sequence = self.pos_encoder(token_sequence)
...
def run_epoch(model, dataloader, criterion, optimizer, device, is_training, epoch_desc=""):
    ...
    with torch.set_grad_enabled(is_training):
        for batch_idx, (sequences, wheel_sequences, targets) in enumerate(progress_bar):
            sequences = sequences.to(device, non_blocking=True)
            wheel_sequences = wheel_sequences.to(device, non_blocking=True)
            targets = targets.to(device, non_blocking=True)
            
            with torch.amp.autocast('cuda', enabled=is_training):
                outputs = model(sequences, wheel_sequences)
...
def predict(model, image_sequence, wheel_sequence, transform, device):
    model.to(device)
    model.eval()
    ...
    wheel_tensor = torch.tensor(wheel_sequence, dtype=torch.float32).unsqueeze(0).to(device)
    
    with torch.no_grad():
        prediction = model(input_tensor, wheel_tensor)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if os.path.exists(BEST_MODEL_SAVE_PATH) and RETRAIN:
            ...
            try:
                ...
                print(f"⚠️ 載入舊模型參數失敗，改為從頭開始訓練: {e}")
    ...
    if len(test_dataset) > 0:
        sample_sequence_tensor, sample_wheel_tensor, true_speeds_tensor = test_dataset[0]
        sample_pil_images = []
        ...
        predicted_speeds = predict(
            model,
            sample_pil_images,
            sample_wheel_tensor.cpu().tolist(),
            val_test_transform,
            device
        )
