...
def place_cones(positions):
    cone_ids = []
    cone_urdf_path = r"D:\Code\3Dmodel\cone.urdf" # 請確認您的路徑
...
def place_trees(positions):
    tree_ids = []
    tree_urdf_path = r"D:\Code\3Dmodel\tree.urdf" # 請確認您的路徑
    for pos in positions:
        try:
...
p.loadURDF(r"D:\Code\3Dmodel\small_track.urdf", basePosition=[0,0,0.055]) # 請確認您的路徑
...
humanoid = p.loadURDF(r"D:\Code\3Dmodel\man.urdf", humanoidStartPos, humanoidStartOrientation) # 請確認您的路徑
...
r2d2 = p.loadURDF(r"D:\Code\3Dmodel\front_car.urdf", r2d2StartPos, r2d2StartOrientation) # 請確認您的路徑
...
try:
    while True:
        ...
        if ord('8') in keys and keys[ord('8')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [2.3, 14.4, 0.35], [0, 0, 0, 1])
        if ord('9') in keys and keys[ord('9')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [0, -1.225, 0.5], [0, 0, 0, 1])            
        if ord('0') in keys and keys[ord('0')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [-7, -15.5, 0.35], [0, 0, 0, 1])  

        # 行人控制
        if ord('2') in keys and keys[ord('2')] & p.KEY_IS_DOWN: is_forward_pressed = True
        elif keys.get(ord('2')) == p.KEY_WAS_RELEASED: is_forward_pressed = False
        if ord('3') in keys and keys[ord('3')] & p.KEY_IS_DOWN: is_backward_pressed = True
        elif keys.get(ord('3')) == p.KEY_WAS_RELEASED: is_backward_pressed = False
        ...
        side_value, wheel_value = 0, 0 # 初始化供 logging 使用
        
        if autodrive_enabled:
            # a. 預設由 Transformer 模型取得駕駛指令
                    ...
                    for box in r.boxes:
                        if float(box.conf[0]) > 0.5:
                            ...
                            if box_w > 150 and box_w > max_box_w:
                                ...
                                max_box_w = box_w
                                closest_obstacle_center_x = (x1 + x2) // 2
            ...
            if retreat:
                ...
                lwheel_value = -retreat_strength
                rwheel_value = -retreat_strength
        ...
        if not autodrive_enabled:
            pass # wheel_value 和 side_value 在手動控制區塊已計算
        else:
