...
# 確保您的 Transformer 模型路徑正確
from trainingmodel.transformer_model_beta5 import PositionalEncoding, VisionTransformerDriver

# --- 1. AI 模型參數 ---
TRANSFORMER_MODEL_PATH = '2best_transformer_driver_model.pth' 
SEQUENCE_LENGTH = 15
IMG_HEIGHT = 224
IMG_WIDTH = 224
D_MODEL = 512
...
def predict_speeds(model, image_sequence, transform, device):
    ...
    return predicted_speeds[0], predicted_speeds[1]

# --- 4. 資料記錄與處理函式 (完整版) ---
def log_data(folder_path, pic_num, img, side_value, wheel_value, lwheel_value, rwheel_value, speed_signed, autodrive_enabled):
    img_name = f"{pic_num:05d}.png"
    img_path = os.path.join(folder_path, 'recorded_images', img_name)
    cv2.imwrite(img_path, img)
    entry = {
        "img_path": img_name, "steering": side_value, "throttle": wheel_value,
        "lwheel": lwheel_value, "rwheel": rwheel_value, "speed_signed": speed_signed,
        "is_autodrive": autodrive_enabled, "timestamp": datetime.datetime.now().isoformat()
    }
...
def split_dataset(csv_path, img_folder, output_folder, val_ratio=0.1, test_ratio=0.1, random_state=42):
    ...
    trainval_df, test_df = train_test_split(df, test_size=test_ratio, random_state=random_state, shuffle=True)
    val_size = val_ratio / (1 - test_ratio)
    train_df, val_df = train_test_split(trainval_df, test_size=val_size, random_state=random_state, shuffle=True)
...
def setup_recording_folders():
    ...
    i = 1
    while True:
        path = os.path.join(day_dir, str(i))
...
# --- 5. PyBullet 場景建置函式 (完整版) ---
def create_zebra_crossing(start_pos, num_lines=6, spacing=0.3, line_size=[2, 0.2, 0.01]):
    for i in range(num_lines):
        basePosition = [start_pos[0], start_pos[1] + i * spacing, start_pos[2]]
        colBoxId = p.createCollisionShape(p.GEOM_BOX, halfExtents=[ls/2 for ls in line_size])
        visBoxId = p.createVisualShape(p.GEOM_BOX, halfExtents=[ls/2 for ls in line_size], rgbaColor=[1,1,1,1])
...
def place_cones(positions):
    cone_ids = []
    cone_urdf_path = "cone.urdf" # 假設 cone.urdf 在 3Dmodel 資料夾中
    for pos in positions:
        try:
            cone_ids.append(p.loadURDF(cone_urdf_path, basePosition=pos))
        except p.error as e:
            print(f"[ERROR] 載入三角錐 URDF 失敗於位置 {pos}。錯誤訊息：{e}")
    return cone_ids

def place_trees(positions):
    tree_ids = []
    tree_urdf_path = "tree.urdf" # 假設 tree.urdf 在 3Dmodel 資料夾中
    for pos in positions:
        try:
            tree_ids.append(p.loadURDF(tree_urdf_path, basePosition=pos))
        except p.error as e:
...
cone_positions = [
    ...
    [7, -14.3, 0.1], [-14, 1, 0.1], [-10, 15, 0.1],
    [12.3, 4.03, 0.1], [-5, -9.25, 0.1]
]
...
CONE_MODEL_PATH = 'runs/detect/yolo_cone_model_v1/weights/best.pt'
try:
    cone_model = YOLO(CONE_MODEL_PATH)
    print(f"成功載入您的「三角錐專家」模型: {CONE_MODEL_PATH}")
except Exception as e:
    print(f"錯誤：載入您的三角錐模型失敗！錯誤訊息: {e}")
    cone_model = None

GENERAL_MODEL_PATH = 'yolo11n.pt'
try:
    general_model = YOLO(GENERAL_MODEL_PATH)
    print(f"成功載入官方「通才專家」模型: {GENERAL_MODEL_PATH}")
except Exception as e:
    print(f"錯誤：載入官方通用模型失敗！錯誤訊息: {e}")
    general_model = None
...
try:
    while True:
        ...
        if ord('5') in keys and keys[ord('5')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [14.2, 25.9, 0.35], [0, 0, 0, 1])
        if ord('6') in keys and keys[ord('6')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [0.6, 14.4, 0.35], [0, 0, 0, 1])
        if ord('7') in keys and keys[ord('7')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [-5.16, 30.94, 0.35], [0, 0, 0, 1])
        if ord('8') in keys and keys[ord('8')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [-12.5, 13.2, 0.35], [0, 0, 0, 1])
        if ord('9') in keys and keys[ord('9')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [0, 1.25, 0.35], [0, 0, 0, 1])
        if ord('0') in keys and keys[ord('0')] & p.KEY_WAS_TRIGGERED: p.resetBasePositionAndOrientation(r2d2, [-17,-9.5, 0.35], [0, 0, 0, 1])

        # 行人控制
        if ord('2') in keys and keys[ord('2')] & p.KEY_IS_DOWN: is_forward_pressed = True
        elif p.KEY_WAS_RELEASED in keys and keys[p.KEY_WAS_RELEASED] == ord('2'): is_forward_pressed = False
        if ord('3') in keys and keys[ord('3')] & p.KEY_IS_DOWN: is_backward_pressed = True
        elif p.KEY_WAS_RELEASED in keys and keys[p.KEY_WAS_RELEASED] == ord('3'): is_backward_pressed = False
        ...
        p.resetDebugVisualizerCamera(cameraDistance=p.readUserDebugParameter(distance), cameraYaw=p.readUserDebugParameter(yaw), cameraPitch=p.readUserDebugParameter(pitch), cameraTargetPosition=r2d2_pos)
        ...
        if ord('s') in keys and keys[ord('s')] & p.KEY_WAS_TRIGGERED:
            save_path = "debug_frame.jpg"
            pil_img.save(save_path)
            print(f"!!! 已儲存當前幀至 {save_path} !!!")

        # --- 2. 執行所有 YOLO 偵測 ---
        cone_results = []
        if cone_model is not None:
            cone_results = cone_model(pil_img, verbose=False)

        general_results = []
        if general_model is not None:
            general_results = general_model(pil_img, verbose=False)
        ...
        if autodrive_enabled:
            if transformer_model is not None and len(image_history) == SEQUENCE_LENGTH:
                lwheel_value, rwheel_value = predict_speeds(transformer_model, list(image_history), ai_transform, device)
            
            obstacle_detected, avoid_direction, retreat = False, 0, False
            
            if cone_results:
                max_box_w = 0
                closest_obstacle_center_x = 0
                for r in cone_results:
                    for box in r.boxes:
                        if float(box.conf[0]) > 0.5:
                            x1, _, x2, _ = map(int, box.xyxy[0].tolist())
                            box_w = x2 - x1
                            if box_w > 150 and box_w > max_box_w:
                                obstacle_detected, max_box_w, closest_obstacle_center_x = True, box_w, (x1 + x2) // 2
                
                if obstacle_detected:
                    if closest_obstacle_center_x < width // 3: avoid_direction = 1
                    elif closest_obstacle_center_x > width * 2 // 3: avoid_direction = -1
                    else:
                        avoid_direction = 1
                        if max_box_w > 300: retreat = True
            
            if retreat:
                retreat_strength = min(30, int((max_box_w - 300) * 0.1))
                lwheel_value = rwheel_value = -retreat_strength
            elif obstacle_detected:
                wheel_value = forward_speed
                rwheel_value = wheel_value * (1 - avoid_direction * d)
                lwheel_value = wheel_value * (1 + avoid_direction * d)
        
        else: # 手動控制
            if p.B3G_UP_ARROW in keys and keys[p.B3G_UP_ARROW] & p.KEY_IS_DOWN: wheel_value = forward_speed
            elif p.B3G_DOWN_ARROW in keys and keys[p.B3G_DOWN_ARROW] & p.KEY_IS_DOWN: wheel_value = -forward_speed
            if p.B3G_LEFT_ARROW in keys and keys[p.B3G_LEFT_ARROW] & p.KEY_IS_DOWN: side_value = -1
            elif p.B3G_RIGHT_ARROW in keys and keys[p.B3G_RIGHT_ARROW] & p.KEY_IS_DOWN: side_value = 1
            ...
            for r in cone_results:
                for box in r.boxes:
                    if float(box.conf[0]) > 0.5:
                        ...
                        label = cone_model.names[int(box.cls[0])]
                        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
                        cv2.putText(img, f"{label} {float(box.conf[0]):.2f}", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            for r in general_results:
                for box in r.boxes:
                    if float(box.conf[0]) > 0.4:
                        ...
                        cv2.putText(img, f"{label} {float(box.conf[0]):.2f}", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        
        linear_velocity, _ = p.getBaseVelocity(r2d2)
        speed_signed = np.dot(np.array(linear_velocity), np.array(camera_forward))
        if not autodrive_enabled:
            pass
        else:
            wheel_value = (actual_lwheel_value + actual_rwheel_value) / 2
            side_value = (actual_lwheel_value - actual_rwheel_value) / (2 * wheel_value * d) if wheel_value != 0 else 0
        
        cv2.putText(img, f"XYZ: ({r2d2_pos[0]:.3f}, {r2d2_pos[1]:.3f}, {r2d2_pos[2]:.3f})", (290, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2, cv2.LINE_AA)
        ...
        for joint in [0, 1, 2, 3]:
            angular_velocity = p.getJointState(r2d2, joint)[1]
            cv2.putText(img, f"Wheel {joint}: {angular_velocity:.2f} rad/s", (10, 20 + joint * 25), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (50, 50, 255), 2)
        cv2.putText(img, f"Car Speed: {speed_signed:.2f} m/s", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 100, 200), 2)
        
        if recording:
            log_data(folder_path, pic_num, img, side_value, wheel_value, actual_lwheel_value, actual_rwheel_value, speed_signed, autodrive_enabled)
            pic_num += 1
    ...
    if folder_path and len(data_log) > 0:
        save_csv_log(folder_path)
        print(f"[INFO] 已儲存 {len(data_log)} 筆模仿學習資料至：{folder_path}/log.csv")
        split_dataset(csv_path=os.path.join(folder_path, "log.csv"), img_folder=os.path.join(folder_path, "recorded_images"), output_folder=folder_path)
