...
MODEL_PATH = 'beta2_at_0829.pth' # 指定訓練好的模型檔案
SEQUENCE_LENGTH = 15
IMG_HEIGHT = 224
IMG_WIDTH = 224
...
def yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, section_x1, section_x2, roi_y1, roi_y2):
    inter_x1 = max(obj_x1, section_x1)
    inter_y1 = max(obj_y1, roi_y1)
    inter_x2 = min(obj_x2, section_x2)
...
if YOLO is not None:
    ...
    try:
        pedestrian_model = YOLO('yolo11n.pt') # yolov8n.pt 是一個輕量且快速的模型
...
# --- 主迴圈初始化 ---
OBSTACLE_CLASSES = ['person', 'truck', 'traffic_cone']  # 定義障礙物類別
# AI 變數
...
try:
    while True:
        ...
        # 將YOLO偵測移至駕駛邏輯之前，以便根據偵測結果進行決策
        # ==================== YOLO 偵測與避障邏輯 ====================
        should_turn_left = False
        should_turn_right = False
        should_brake = False
        results = pedestrian_model(img, verbose=False)
        if pedestrian_model and autodrive_enabled:
            # 在畫面中央畫一個300*200的方形
            center_x, center_y = width // 2, height // 2
            box_size_x = 300
            box_size_y = 200
            roi_x1 = center_x - box_size_x // 2
            roi_y1 = center_y - box_size_y // 2
            roi_x2 = center_x + box_size_x // 2
            roi_y2 = center_y + box_size_y // 2
            
            # 定義ROI的三個分區
            section_width = box_size_x // 3
            left_section_x1 = roi_x1
            left_section_x2 = roi_x1 + section_width
            mid_section_x1 = roi_x1 + section_width
            mid_section_x2 = roi_x1 + 2 * section_width
            right_section_x1 = roi_x1 + 2 * section_width
            right_section_x2 = roi_x2
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    if class_name in OBSTACLE_CLASSES:
                        ...
                        section_roi_area = section_width * (roi_y2 - roi_y1)
                        if section_roi_area == 0: continue

                        # 計算與各區域的交集比例
                        mid_intersection = yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, mid_section_x1, mid_section_x2, roi_y1, roi_y2)
                        right_intersection = yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, right_section_x1, right_section_x2, roi_y1, roi_y2)
                        left_intersection = yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, left_section_x1, left_section_x2, roi_y1, roi_y2)
                        
                        mid_proportion = mid_intersection / section_roi_area
                        right_proportion = right_intersection / section_roi_area
                        left_proportion = left_intersection / section_roi_area
                        
                        if class_name == 'person' and max(mid_proportion, right_proportion, left_proportion) != 0:
                            people_detected = True
                            should_brake = True
                        # 設定避障旗標
                        if right_proportion > 0.05: # 右邊有障礙物，需要左轉
                            should_turn_left = True
                        if left_proportion > 0.05: # 左邊有障礙物，需要右轉
                            should_turn_right = True
                        if mid_proportion > 0.45: # 中間障礙物過大，需要剎車
                            should_brake = True
        # ===============================================================
        ...
        if autodrive_enabled and ai_model is not None:
            ...
            if should_brake:
                lwheel_value, rwheel_value = 0, 0
                if people_detected:
                    print("[WARNING] AI 偵測到行人，緊急剎車！")
                else:
                    print("[WARNING] AI 偵測到前方障礙物，自動剎車！")
            elif should_turn_left and not should_turn_right: # 只在右邊有障礙物時左轉
                lwheel_value, rwheel_value = -15, 15 # 左轉
                print("[INFO] AI 偵測到右側障礙物，自動左轉。")
            elif should_turn_right and not should_turn_left: # 只在左邊有障礙物時右轉
                lwheel_value, rwheel_value = 15, -15 # 右轉
                print("[INFO] AI 偵測到左側障礙物，自動右轉。")
            elif len(image_history) == SEQUENCE_LENGTH:
                # 若無須避障，則使用模型預測
        ...
        if pedestrian_model:
            ...
            roi_x2 = center_x + box_size_x // 2
            roi_y2 = center_y + box_size_y // 2
            cv2.rectangle(img, (roi_x1, roi_y1), (roi_x2, roi_y2), (0, 255, 0), 2) # 綠色方框
            cv2.putText(img, 'ROI', (roi_x1, roi_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2) # 在方框左上角標註 ROI

            # 用垂直線將ROI水平平分3等分，並定義中間區域
            section_width = box_size_x // 3
            mid_section_x1 = roi_x1 + section_width
            mid_section_x2 = roi_x1 + 2 * section_width
            cv2.line(img, (mid_section_x1, roi_y1), (mid_section_x1, roi_y2), (0, 255, 0), 2)
            cv2.line(img, (mid_section_x2, roi_y1), (mid_section_x2, roi_y2), (0, 255, 0), 2)
            ...
            for r in results:
                ...
                for box in boxes:
                    ...
                    # 只處理在 OBSTACLE_CLASSES 中的物件
                    if class_name in OBSTACLE_CLASSES:
                        obj_x1, obj_y1, obj_x2, obj_y2 = map(int, box.xyxy[0])
                        
                        # 2. 計算物體框與ROI中間區域的交集面積
                        mid_intersection_area = yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, mid_section_x1, mid_section_x2, roi_y1, roi_y2)
                        r_intersection_area = yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, mid_section_x2, roi_x2, roi_y1, roi_y2)
                        l_intersection_area = yolo_detec_area(obj_x1, obj_y1, obj_x2, obj_y2, roi_x1, mid_section_x1, roi_y1, roi_y2)
                        
                        # 3. 計算比例 (交集面積 / ROI各區塊面積)
                        section_roi_area = (mid_section_x2 - mid_section_x1) * (roi_y2 - roi_y1)
                        if section_roi_area == 0: continue
                        mid_proportion = mid_intersection_area / section_roi_area
                        right_proportion = r_intersection_area / section_roi_area
                        left_proportion = l_intersection_area / section_roi_area
                        
                        # 繪製方框
                        cv2.rectangle(img, (obj_x1, obj_y1), (obj_x2, obj_y2), (255, 0, 255), 2)
                        
                        # 動態組合標籤文字
                        label_parts = [f'{class_name} {confidence}']
                        if left_proportion > 0:
                            label_parts.append(f'L:{left_proportion:.1%}')
                        if mid_proportion > 0:
                            label_parts.append(f'M:{mid_proportion:.1%}')
                        if right_proportion > 0:
                            label_parts.append(f'R:{right_proportion:.1%}')
                        
                        label = ' '.join(label_parts)
                                                
                        (w, h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
                        cv2.rectangle(img, (obj_x1, obj_y1 - h - 5), (obj_x1 + w, obj_y1), (255, 0, 255), -1) # 繪製標籤背景 
                        # 繪製標籤文字
                        cv2.putText(img, label, (obj_x1, obj_y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
