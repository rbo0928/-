...
p.loadURDF(r"D:\Code\small_track.urdf", basePosition=[0,0,0.055])
...
try:
    while True:
        ...
        if ord('0') in keys and keys[ord('0')] & p.KEY_WAS_TRIGGERED:
            p.resetBasePositionAndOrientation(r2d2, [-5, -15.5, 0.35], [0, 0, 0, 1])
        if ord('9') in keys and keys[ord('9')] & p.KEY_WAS_TRIGGERED: #小賽道
