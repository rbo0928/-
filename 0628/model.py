...
if __name__ == '__main__':
    ...
    # 建立資料集和資料載入器
    dataset = DrivingDataset(csv_file=CSV_PATH, img_dir=IMG_DIR, sequence_length=SEQUENCE_LENGTH, transform=transform)
    dataloader = DataLoader(dataset=dataset, batch_size=BATCH_SIZE, shuffle=True)

    # 初始化模型、損失函數和優化器
    model = CNNLSTM()
    criterion = nn.MSELoss() # 均方誤差損失，適合回歸問題
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=LEARNING_RATE)

    # 訓練模型
    train_model(model, dataloader, criterion, optimizer, device, num_epochs=EPOCHS)
